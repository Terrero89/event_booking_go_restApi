...
func createEvent(context *gin.Context) {
	...
	userId := context.GetInt64("userId") //retrieve userID from auth as a global param
	event.UserID = userId
...
func deleteEvent(context *gin.Context) {
	...
	//comparing if user id is the one that created this event.
	//before of delete method
	if event.UserID != userId {
		context.JSON(http.StatusUnauthorized, gin.H{"message": "You are not authorized to update this event."})
		return
	}
